...
const deployFunction: DeployFunction = async ({ getNamedAccounts, deployments }) => {
  ...
  if (!developmentChains.includes(network.name) && process.env.ETHERSCAN_API_KEY) {
    log("Verifying...")
    await verify(randomNumberGeneratorV2.address, args)
  }

  log("Run RandomNumberGeneratorV2 contract with the following command")
  const networkName = network.name == "hardhat" ? "localhost" : network.name
  log(
    `yarn hardhat request-random-number --contract ${randomNumberGeneratorV2.address} --network ${networkName}`
  )
  log("----------------------------------------------------")
}
